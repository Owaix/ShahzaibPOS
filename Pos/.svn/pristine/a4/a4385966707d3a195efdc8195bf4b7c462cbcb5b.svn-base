...
{
    public partial class NotPaidCustomerByMonth : MetroFramework.Forms.MetroForm
    {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            var getcustomers = db.Customers.ToList();
            var getRecord = db.getPaidRecordByMonth(To, From).ToList();
            var count = getRecord.Count();
            bool check = false;
            for (int a = 0; a < getcustomers.Count(); a++)
            {
                for (int b = 0; b < count; b++)
                {
                    //var abc = new MyModels.VendorLedger();

                    if (getcustomers[a].AC_Code == getRecord[b].AC_Code)
                    {
                        check = true;
                    }
                ...
                if (check == false) {
                    ...
                    CategorysDataGridView.Rows.Add(getcustomers[a].CusName,0);
