...
namespace SalesMngmt.Invoice
{
    public partial class SInv : MetroFramework.Forms.MetroForm
    {
        ...
        public SInv(int Company)
        {
            ...
            listItems = db.Items.Where(x => x.CompanyID == compID).ToList();
        }
        ...
        private void SInv_Load(object sender, EventArgs e)
        {
            ...
            parties.AddRange(db.COA_D.Where(x => x.CAC_Code == 1 && x.CompanyID == compID).ToList());
            //   var vendor = ;
            ...
            cash.AddRange(db.COA_D.Where(x => x.CAC_Code == 1 && x.CompanyID == compID).ToList());
        ...
        private void SaveRecord(String InvoiceNo)
        {
            ...
            try
            {
                ...
                foreach (var itemName in list)
                {
                    ...
                    orders.GrossAmt = TotalGross;
                    orders.DiscountTotal = Amount - Convert.ToDouble(order.DisAmt.DefaultZero());
                    orders.TotalDiscount = Convert.ToDecimal(TotalGross) - Convert.ToDecimal(Amount) - Convert.ToDecimal(order.DisAmt.DefaultZero());  //(Convert.ToDecimal(itemName.SalesPriceP) * Convert.ToDecimal( TOtalValue))- Convert.ToDecimal(itemName.Amt);
                ...
                orders1.TotalDiscount = Convert.ToDecimal(TotalGross) - Convert.ToDecimal(Amount) - Convert.ToDecimal(order.DisAmt);  //(Convert.ToDecimal(itemName.SalesPriceP) * Convert.ToDecimal( TOtalValue))- Convert.ToDecimal(itemName.Amt);
        ...
        private void textBox1_Leave(object sender, EventArgs e)
        {
            if (cmbxItems.Text.Trim() != string.Empty)
            {
                ...
                lblStock.Text = db.getStockByID(item.IID).FirstOrDefault().ToString();
