...
namespace SalesMngmt.Reporting
{
    public partial class ItemSummary : Form
    {
        ...
        public ItemSummary(int _compID)
        {
            compID = _compID;
        }
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            var rows = ReportsController.GetItemSummary(Vendorcode, dtTOdate.Value, dtFrom.Value, compID);
            var getdata = rows.AsEnumerable().Select(i => new
            {
                Name = i["IName"].ToString(),
                Quantity = Convert.ToInt16(i["Qty"]),
                Amount = Convert.ToInt16(i["Qty"]) * Convert.ToDecimal(i["Rate"])
            }).ToList();
