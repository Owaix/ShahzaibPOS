...
namespace SalesMngmt.Configs
{
    public partial class Catgory : MetroFramework.Forms.MetroForm
    {
        ...
        {
            pnlMain.Hide();
            list = db.Items_Cat.AsNoTracking().Where(x => x.CompanyID == compID).ToList();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                var Currentobj = db.Items_Cat.Where(x => x.Cat == txtCategory.Text.Trim() && x.CompanyID == compID).FirstOrDefault();
                if (obj.CatID == 0)
                {
                    if (Currentobj != null)
                    {
                        MessageBox.Show("Catergory Name Already Exists", "Duplicate", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    }
                }
                else
                ...
                {
                    var result = db.Items_Cat.SingleOrDefault(b => b.CatID == obj.CatID && b.CompanyID == compID);
                    if (result != null)
                    {
                        result.Cat = txtCategory.Text.Trim();
                        result.isDeleted = chkIsActive.Checked;
                        result.img = filename;
                        result.Images = Utillityfunctions.ToBase64(openFileDialog1.FileName, path + "\\Img\\" + filename);
                    }
                }
                db.SaveChanges();
                pnlMain.Hide();
