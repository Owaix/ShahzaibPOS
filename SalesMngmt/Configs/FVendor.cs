...
{
    public partial class FVendor : MetroFramework.Forms.MetroForm
    {
        SaleManagerEntities db = null;
        List<Vendors> list = null;
        int compID = 0;
        public FVendor(int comID)
        {
            InitializeComponent();
        ...
        private void FVendor_Load(object sender, EventArgs e)
        {
            pnlMain.Hide();
            list = db.Vendors.AsNoTracking().Where(x => x.CompanyID == compID).ToList();
            VendBindingSource.DataSource = list;
            FillCombo(cmbPType, db.Party_Type.AsNoTracking().Where(x => x.CompanyID == compID && x.isDeleted==false).ToList(), "Party_Type1", "PType_ID");
            FillCombo(cmbxZone, db.Zone.Where(x => x.CompanyID == compID).ToList(), "Zone1", "ZID");
        }

        private void lblAdd_Click(object sender, EventArgs e)
        {
            VendBindingSource.AddNew();
            pnlMain.Show();
            txtVndrName.Focus();
            label3.Text = "ADD";
        }
        ...
        {
            Vendors obj = (Vendors)VendBindingSource.Current;
            pnlMain.Show();
            txtVndrName.Focus();
            label3.Text = "EDIT";
        }
        ...
        {
            pnlMain.Hide();
            Vendors us = (Vendors)VendBindingSource.Current;
            if (us.VID == 0)
            {
                VendBindingSource.RemoveCurrent();
            }
            else
            {

                VendBindingSource.Clear();
                var listcancel = db.Vendors.AsNoTracking().Where(x=>x.CompanyID==compID).ToList();
                VendBindingSource.DataSource = listcancel;
            }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    var Currentobj = db.Vendors.ToList().Find(x => x.VendName == txtVndrName.Text.Trim() && x.CompanyID==compID);
                    if (obj.VID == 0)
                    {
                        if (Currentobj != null)
                        {
                    ...
                    obj.Debit = Convert.ToDouble(txtDebit.Text.Trim());
                    obj.Credit = Convert.ToDouble(txtCredit.Text.Trim());
                    obj.CompanyID = compID;
                    obj.PType_ID= Convert.ToInt32(cmbPType.SelectedValue);
                    ...
                    {
                        var result = db.Vendors.SingleOrDefault(b => b.VID == obj.VID && b.CompanyID==compID);
                        if (result != null)
                        {
                            ...
                            result.Debit = Convert.ToDouble(txtDebit.Text.Trim());
                            result.Credit = Convert.ToDouble(txtCredit.Text.Trim());
                            result.PType_ID = Convert.ToInt32(cmbPType.SelectedValue);
                        }
                    }
                    db.SaveChanges();
                    ...
                    gl.Debit = Convert.ToDouble(txtDebit.Text.Trim());
                    gl.Credit = Convert.ToDouble(txtCredit.Text.Trim());
                    gl.CompID = compID;
                    if (!isAdd)
                    {
                    ...
                    gl2.Narration = "Opening "+txtVndrName.Text.Trim();
                    gl2.Debit = Convert.ToDouble(txtCredit.Text.Trim());
                    gl2.Credit = Convert.ToDouble(txtDebit.Text.Trim());
                    gl2.CompID = compID;
                    if (!isAdd)
                    {
        ...
        {
            VendDataGridView.SelectionMode = DataGridViewSelectionMode.FullRowSelect;
        }
        ...
        private void toolStripTextBoxFind_Leave(object sender, EventArgs e)
        {
            try
            {
                if (toolStripTextBoxFind.Text.Trim().Length == 0) { VendDataGridView.DataSource = list; }
                else
                {
                    VendDataGridView.DataSource = list.FindAll(x => x.VendName.ToLower().Contains(toolStripTextBoxFind.Text.ToLower().Trim()));
                }
            }
            catch (Exception)
            {
        ...
        private void VendDataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 14)
            {

                int code = Convert.ToInt32(VendDataGridView.Rows[e.RowIndex].Cells[0].Value);

                Reporting.vendorLedgerSummary ledger = new Reporting.vendorLedgerSummary();
